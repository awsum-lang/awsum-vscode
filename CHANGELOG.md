...
No user-facing changes to the extension itself; this release tracks the compiler version bump. The TextMate grammar is unchanged — every 0.0.6 language change is in the compiler and surfaces through `awsum lsp`.
...
- **Migrated to Language Server Protocol.** `awsum-vscode` is now a thin client to `awsum lsp --stdio` (subcommand of the compiler). Diagnostics, code actions, formatting, document and workspace symbols all come from the server — no more per-request CLI spawn, temp files, debouncer, or `awsum --version` subprocess. Declarative lockstep version check: the extension passes `initializationOptions: { expectedAwsumVersion, preferButtonsOverLinks: true }` and the server warns on mismatch via `window/showMessageRequest` with an action button that opens the install page through `window/showDocument`. Setting renamed `awsum.format.path` → `awsum.path` — it always named the binary, and that binary now hosts the LSP server too.
